...
from auxiliary_tasks.experience import Transaction
...
def play_game(model, env):
    ...
    if model.config.train.use_value_replay:
        stats['vr_loss'] = model.vr_loss().detach().numpy()
...
def record_video(model, env):
    ...
    if model.config.train.use_pixel_control:
        result.update({'pc_loss': stats['pc_loss']})
    if model.config.train.use_reward_prediction:
        result.update({'rp_loss': stats['rp_loss']})
    if model.config.train.use_value_replay:
        result.update({'vr_loss': stats['vr_loss']})
    return result
